...
const GroupUpsert = () => {
    ...
    useEffect(() => {
        ...
        if (id !== undefined){
              const fetchData = async () => {
                try {
                  const response = await consume_service(`${URL_GROUPS_BASE}${id}`,'get','',{},false);
    ...
    const handleSubmit = async (event) => {
        ...
        try {            
            ...
            if (id !== undefined){
                await consume_service(`${URL_GROUPS_BASE}${id}`,'put',
                            jwtToken,formData,true);
            }
            else {
                await consume_service(URL_GROUPS_BASE,'post',jwtToken,formData,true);
             }
